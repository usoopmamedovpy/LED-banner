// ============================================
// LED BANNER - WYSIWYG MATH EDITOR (ИСПРАВЛЕННЫЙ)
// ============================================
...
// ============================================
// ВСТАВКА В MATHQUILL - LaTeX-ориентированная
// ============================================
function insertMathCommand(latexCmd) {
    if (!mathField || !latexCmd) return;
    
    try {
        console.log('Inserting LaTeX command:', latexCmd);
        
        // Обрабатываем частые случаи
        if (latexCmd === '\\frac') {
            mathField.cmd('\\frac'); // создает дробь с пустыми местами
        } else if (latexCmd === '\\sqrt') {
            mathField.cmd('\\sqrt'); // создает корень
            ...
            // Для всех остальных команд (sin, cos, греческие буквы и т.д.)
            mathField.cmd(latexCmd);
        }
...
// ============================================
// КНОПКИ КЛАВИАТУРЫ - ИСПРАВЛЕНО (берем data-cmd)
// ============================================
...
mathKeys.forEach(function(btn) {
    btn.addEventListener('click', function(e) {
        ...
        const latexCmd = this.dataset.cmd; // ВАЖНО: берем data-cmd, а не textContent!
        if (latexCmd) {
            insertMathCommand(latexCmd);
        }
...
console.log('✅ LED BANNER - WYSIWYG MATH EDITOR (FIXED)');
